...
<b>Melhor espaçamento superado:</b> ${displayBestSpacing(w)} · barreira normal ${normalSpacingThreshold()}<br><b>Estado:</b> ${wordStateTag(w)}<br><b>Prioridade:</b> ${wordPriorityTag(w)}<br><b>Barreira:</b> ${barrierSummary(w)}<br>
...
function buildPracticeQueue(){
 ...
 if(mode==="reviews"){
   ...
   const hard=active
     .filter(w=>isViableCard(w) && isHardModeCandidate(w) && !isConfirmationState(w))
     .sort(hardModeSort);
   selected=takeBalanced(hard, targetUnique);
...
function visualTagLabel(word){
 return wordStateTag(word);
}


function getWordState(word){
 try{
   if(!word) return {key:"unknown", label:"—", emoji:"", text:"—"};
   const xp = Number(word.xp || 0);
   const memory = Number(word.memoryLevel || 0);
   const stability = Number(word.stabilityScore || 0);
   const streak = Number(word.correctStreak || 0);
   if(xp<=0 && memory<=0 && stability<=0 && streak<=0) return {key:"new", label:"Nova", emoji:"⚪", text:"⚪ Nova"};
   if(isFragile(word)) return {key:"fragile", label:"Frágil", emoji:"🔴", text:"🔴 Frágil"};
   if(isConfirmationState(word)) return {key:"verification", label:"Verificação", emoji:"🟡", text:"🟡 Verificação"};
   if(isResistant(word)) return {key:"resistant", label:"Resistente", emoji:"🟠", text:"🟠 Resistente"};
   if(isMasteredForScheduling(word)) return {key:"review", label:"Revisão", emoji:"🟢", text:"🟢 Revisão"};
   return {key:"learning", label:"Aprendizagem", emoji:"🔵", text:"🔵 Aprendizagem"};
 }catch(e){ return {key:"unknown", label:"—", emoji:"", text:"—"}; }
}

function getWordPriority(word){
 try{
   if(!word) return {key:"normal", label:"Normal", emoji:"", text:"Normal"};
   if(isCritical(word)) return {key:"critical", label:"Crítica", emoji:"⚠️", text:"⚠️ Crítica"};
   return {key:"normal", label:"Normal", emoji:"", text:"Normal"};
 }catch(e){ return {key:"normal", label:"Normal", emoji:"", text:"Normal"}; }
}

function wordStateTag(word){ return getWordState(word).text; }
function wordPriorityTag(word){ return getWordPriority(word).text; }


function isHardModeCandidate(word){
 return isResistant(word) || isFragile(word);
}

function hardModeRank(word){
 if(isResistant(word)) return 0;
 if(isFragile(word)) return 1;
 return 9;
}

function hardModeSort(a,b){
 const ar=hardModeRank(a), br=hardModeRank(b);
 if(ar!==br) return ar-br;
 if(ar===0) return (a.xp||0)-(b.xp||0);
 if(ar===1) return clampFragilityDebt(b.fragilityDebt||0)-clampFragilityDebt(a.fragilityDebt||0);
 return (a.memoryLevel||0)-(b.memoryLevel||0);
}

function matchesDbFilter(word, filter){
 const state = getWordState(word).key;
 switch(filter){
   case "all": return true;
   case "words": return !isPhraseCard(word);
   case "phrases": return isPhraseCard(word);
   case "new": return state==="new";
   case "learning": return state==="learning";
   case "resistant": return state==="resistant";
   case "review": return state==="review";
   case "verification": return state==="verification";
   case "fragile": return state==="fragile";
   case "hardSmart": return isHardModeCandidate(word);
   case "critical": return getWordPriority(word).key==="critical";
   case "due": return isReviewCandidateOrMissingDate(word) && (!word.nextReviewAt || isDueForReview(word));
   case "confirmation": return state==="verification";
...
function renderProgress(){
 try{
   const tagEl=$("trainingStateTag");
   if(tagEl){
     tagEl.textContent = currentCard ? `${wordStateTag(currentCard)}${wordPriorityTag(currentCard)!=="Normal" ? " · "+wordPriorityTag(currentCard) : ""}` : "";
   }
 }catch(e){}
...
if(filterMode!=="all"){
 list=list.filter(x=>matchesDbFilter(x.item, filterMode));
}
 ...
 try{
   ...
   const finish = (text)=>{
     ...
     if(payload.settings && typeof applyImportedSettings === "function"){
       try{ settingsApplied=!!applyImportedSettings(payload.settings); }catch(e){ console.warn("settings import ignored", e); }
     }

     safeClearImportBoxes();
     setImportFeedback(`Backup importado: ${result.added} novos · ${result.updated} atualizados · ${result.invalid} inválidos. Total: ${result.after}.${settingsApplied ? " Configurações restauradas." : " Sem configurações no backup."}`, true);
...
const SETTINGS_CONTROL_IDS = ["adaptiveScheduler", "autoMicDelay", "autoMicEnabled", "autoMicMaxSilentMinutes", "backupReminder", "criticalThreshold", "initialReviewDays", "masterThreshold", "maxCardsPerSession", "maxNewWords", "memoryArticleError", "memoryCorrect", "memoryGrammarError", "memoryMemoryError", "minExposuresToMaster", "minMemoryLevel", "minPhrasesPerSession", "minRepeatDistance", "minReviewsPerSession", "minStreakToMaster", "modeHardCards", "modeHardMemoryMult", "modeHardSpacingMult", "modeHardStabilityMult", "modeHardXpMult", "modeIntenseCards", "modeIntenseMemoryMult", "modeIntenseNewPct", "modeIntenseSpacingMult", "modeIntenseStabilityMult", "modeIntenseXpMult", "modeNormalCards", "modeNormalMemoryMult", "modeNormalSpacingMult", "modeNormalStabilityMult", "modeNormalXpMult", "modePhrasesActivePct", "modePhrasesCards", "modePhrasesMemoryMult", "modePhrasesSpacingMult", "modePhrasesStabilityMult", "modePhrasesXpMult", "modeReviewsCards", "modeReviewsDuePct", "modeReviewsMemoryMult", "modeReviewsSpacingMult", "modeReviewsStabilityMult", "modeReviewsXpMult", "repeatCriticalWords", "repeatSpacingMode", "requireArticle", "reviewGrowthFactor", "sessionExcludedThreshold", "showWordInspector", "spacingHighMax", "spacingHighMin", "spacingLowMax", "spacingLowMin", "spacingMidMax", "spacingMidMin", "spacingVeryHighMax", "spacingVeryHighMin", "speakArticle", "speakPortuguese", "speechEnabled", "stabilityArticleError", "stabilityCorrect", "stabilityDueCorrect", "stabilityDueMemoryError", "stabilityGrammarError", "stabilityMasterThreshold", "stabilityMemoryError", "strictGrammarRecognition", "targetDistinctWords", "voiceDebug", "voiceEnabled", "voiceStrictness", "waitForPortugueseBeforeMic", "xpArticleError", "xpCorrect", "xpDueCorrect", "xpDueMemoryError", "xpGrammarError", "xpMemoryError", "xpSkip"];
...
function collectSettings(){
 ...
 SETTINGS_CONTROL_IDS.forEach(id=>{
   const el=$(id);
   if(el) settings[id]=readControlValue(el);
 });
 return settings;
}
 ...
 try{
   ...
   SETTINGS_CONTROL_IDS.forEach(id=>{
     if(Object.prototype.hasOwnProperty.call(settings,id)){
       writeControlValue($(id), settings[id]);
     }
   });
   saveSettings();
   if(typeof updateTrainingModeInfo === "function") updateTrainingModeInfo();
   if(typeof renderWordList === "function") renderWordList();
   if(typeof updateStats === "function") updateStats();
   return true;
 }catch(e){
   console.warn("applyImportedSettings failed", e);
   return false;
 }
}
...
function loadSettings(){
 try{
   ...
   console.warn("loadSettings failed; keeping HTML defaults", e);
 }
}
